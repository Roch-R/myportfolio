...
/* ─── Reveal  (NO willChange / NO transform — these create stacking contexts
               that clip absolutely-positioned orbit logos) ─── */
...
/* ─── TiltCard  (NO transformStyle:preserve-3d — breaks ElectricBorder canvas) ─── */
        ...
        <div style={{ padding: "36px 40px 40px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
            ...
            <button onClick={onClose}
              ...
              onMouseEnter={e => { e.currentTarget.style.background = "#1a1a2e"; e.currentTarget.style.borderColor = "#1a1a2e"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "#fff"; e.currentTarget.style.borderColor = "#e8e6de"; }}>
...
export default function Portfolio() {
  ...
  /* ── ResizeObserver on the orbit column — gives us the REAL pixel width
         so baseWidth always matches what OrbitImages actually renders into ── */
  ...
  /* Orbit dimensions derived from actual column width */
  const oRx   = Math.round(colW * 0.36);          // horizontal orbit radius
  const oRy   = Math.round(colW * 0.43);          // vertical orbit radius
  const oItem = Math.min(44, Math.max(22, Math.round(colW * 0.088))); // logo icon size
    ...
    <div style={s.root}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .hero-grid{
          display:grid;
          /* Left col gets text, right col gets orbit.
             Right column width = colW (measured by ResizeObserver).
             We can't know colW at CSS time so we let the grid be 1.2fr/1fr and
             the JS ResizeObserver measures the actual rendered right-col width. */
          grid-template-columns: 1.2fr 1fr;
          gap: 60px;
          align-items: center;
          /* overflow MUST be visible — orbit logos extend past the card */
          overflow: visible;
        }

        /* ═══ ORBIT COLUMN — everything inside must allow overflow ═══ */
        .orbit-col{
        ...
        /* Explicitly override every descendant — OrbitImages may add its own containers */
        .orbit-col > *,
        .orbit-col > * > *,
        .orbit-col > * > * > *,
        .orbit-col > * > * > * > * {
        ...
        /* ═══ Mobile ≤768px ═══
             Keep 2 columns — image stays on RIGHT.
             Shrink text so it fits in the left column.
        ════════════════════════ */
        @media(max-width:768px){
          ...
          .metric-label{font-size:7px !important;margin-top:3px !important}
      ...
      <section id="Home" className="hero-section-mobile" style={s.hero}>
        ...
        <div style={s.heroContainer}>
          <div className="hero-grid">
            ...
            {/* ── RIGHT: orbit ──
                 ref on the outer div → ResizeObserver reads actual pixel width
                 → baseWidth always matches the real container → logos render correctly
                 No Reveal wrapper here — any opacity/transform ancestor creates a
                 stacking context that clips position:absolute orbit logos
            ── */}
            <div className="orbit-col" ref={orbitColRef}>
              {/* colW > 0 guard prevents a flash render with wrong baseWidth */}
              {colW > 0 && (
                <TiltCard maxTilt={6}>
                  <OrbitImages
                    ...
                    rotation={0}
                    centerContent={
                      /* inline-block + no overflow:hidden so ElectricBorder canvas (which
                         is absolutely positioned 30px outside the card) stays visible */
                  ...
                  <div>
                    <div className="form-row" style={s.formRow}>
                      <div style={s.inputGroup}>
                        ...
                        <input className="form-input" type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} style={s.input} placeholder="John Doe" />
                      </div>
                      <div style={s.inputGroup}>
                        <label style={s.label}>Email Address</label>
                        <input className="form-input" type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} style={s.input} placeholder="john@company.com" />
                      </div>
                    </div>
...
const s = {
  ...
  /* NO overflow:hidden on hero — orbit logos extend past section boundaries */
